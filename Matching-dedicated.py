...
def newMatchset():
    ...
    master = pd.DataFrame(columns=('id','zpdf','mass','AGN or not')) # initialises an empty data frame for the sampled data to be appended to
    ...
    for i_x in agn_histogram:
        ...
        for i_y in i_x: # These nested loops loop through the histogram to see if there are more or less agns
            ...
            if i_y == 0: # begins checking the amount of agns in the square with 0 (to speed up process)
                print('\r','matches made: ',matchesmade,'; Skipped data at:',[x,y],'                    ',sep='',end='', flush = True)
                ...
                gal_sample['AGN or not'] = gal_type
                agn_subset['AGN or not'] = agn_type
                ...
                matchesmade += 2*i_y
                print('\r','matches made: ',matchesmade,'; ',i_y,' AGNs and ', gal_histogram[x,y], ' Galaxies at ',[x,y],'                    ',sep='',end='', flush = True)
                time.sleep(0.05)
                ...
                gal_subset['AGN or not'] = gal_type
                agn_subset['AGN or not'] = agn_type
                ...
                matchesmade += 2*i_y
                print('\r','matches made: ',matchesmade,'; ',i_y,' AGNs and ', gal_histogram[x,y], ' Galaxies at ',[x,y],'                    ',sep='',end='', flush = True)
                time.sleep(0.05) # A note for another time: this whole progress thing would look alot nicer with the matches made bit at the front (not of particularly variable width)
